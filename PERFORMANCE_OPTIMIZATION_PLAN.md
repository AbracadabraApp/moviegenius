...
async function getCachedAnalysis(movieId) {
  ...
  await redis.setex(`analysis:${movieId}`, CACHE_DURATION.MOVIE_ANALYSIS, JSON.stringify(analysis));
  return analysis;
}
```
...
export default {
  async fetch(request) {
    ...
    if (request.url.includes('/images/') || request.url.includes('/_next/static/')) {
...
function MovieGrid({ movies }) {
  ...
  return (
    <div ref={gridRef}>
      {isVisible && movies.map(movie => <MovieCard key={movie.id} movie={movie} />)}
    </div>
  );
}
```
...
This plan prioritizes changes that will have the most immediate impact on user perception while building toward a comprehensive performance optimization strategy.