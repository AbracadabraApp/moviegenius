...
export default function CinemaThroughTime() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const containerHeight = document.getElementById('cinema-container')?.offsetHeight || window.innerHeight
      const scrollPercent = currentScrollY / (containerHeight * 0.8)
      const newEra = Math.min(Math.max(Math.floor(scrollPercent * cinemaEras.length), 0), cinemaEras.length - 1)
