...
class ProdMovieTestFramework {
  ...
  log(message, data = {}) {
    ...
    console.log('[PROD-MOVIE-TEST]', logEntry);
  }
  ...
  async generateValidationReport() {
    ...
    if (this.config.reportStorage === 'localStorage') {
      localStorage.setItem('prodMovieTestReport', JSON.stringify(report));
    }

    console.log('📊 PRODUCTION MOVIE VALIDATION REPORT:', report);
  ...
  init() {
    this.log('🚀 Production movie testing framework initializing', {
      config: this.config,
      url: window.location.href
    });
...
if (typeof window !== 'undefined') {
  const tester = new ProdMovieTestFramework({
  ...
  window.generateProdMovieTestReport = () => tester.generateValidationReport();
  window.getProdMovieTestLogs = () => tester.logs;
  window.prodMovieTestFramework = tester;
}