...
async function testBothModels() {
  ...
  try {
    ...
    haikuExplore.forEach((ex, i) => console.log(`${i+1}. ${ex.replace('EXPLORE_FURTHER: ', '')}`));
    console.log('\nSONNET EXPLORE FURTHER:');
    sonnetExplore.forEach((ex, i) => console.log(`${i+1}. ${ex.replace('EXPLORE_FURTHER: ', '')}`));
    ...
    function extractParagraphWords(content) {
      ...
      const paragraphText = paragraphLines.map(line => line.replace(/^PARAGRAPH:\s*/, '').trim()).join(' ');
    ...
    function scoreExploreFurther(exploreItems) {
      ...
      items.forEach(item => {
        ...
        if (item.includes('technique') || item.includes('influence') || item.includes('evolution')) qualityScore += 0.5;
        if (!item.toLowerCase().includes('how did') && !item.toLowerCase().includes('what is')) qualityScore += 0.5;
        
        // Relevance: extends naturally from content
        if (item.includes('German') || item.includes('Expressionism') || item.includes('noir') || item.includes('lighting') || item.includes('émigré')) relevanceScore += 1;
        
        // Specificity: avoid generic questions
        if (!item.toLowerCase().includes('how did x influence y') && item.length > 30) specificityScore += 1;
      });
    ...
    function scoreSubheads(subheadItems) {
      ...
      items.forEach((item, index) => {
        ...
        if (item.includes('Visual') || item.includes('Psychological') || item.includes('Technical') || item.includes('Influence')) organizationScore += 1;
        
        // Clarity: help readers navigate
        if (item.length < 50 && item.length > 10) clarityScore += 1;
        
        // Engagement: intriguing rather than academic
        if (!item.includes('Analysis') && !item.includes('Overview') && !item.includes('Introduction')) engagementScore += 1;
        
        // Logic: coherent progression (later subheads should build)
        if (index > 0 && (item.includes('Influence') || item.includes('Legacy') || item.includes('Evolution'))) logicScore += 1;
      });
