...
Movie detail pages were returning 404 errors for TMDB IDs that exist in TMDB but
not in our local database (e.g., `/movie/39206`, `/movie/10331`).

## Root Cause

1. **Static Generation**: `getStaticPaths` was set to `fallback: false`,
   preventing dynamic generation
2. **Database Dependency**: `getStaticProps` returned `notFound: true` for
   movies not in database
3. **API Limitation**: `load-movie-page` API didn't handle missing movies
   gracefully
...
if (movieError || !movie) {
  console.log(
    `Movie ${tmdb_id} not found in database, attempting to create from TMDB...`
  );
...
2. **Static generation** checks if path exists, uses `fallback: 'blocking'` for
   missing paths
3. **getStaticProps** returns placeholder props with
   `title: 'TMDB_FETCH_REQUIRED'`
...
**Before**: 404 error for movies not in database **After**:
