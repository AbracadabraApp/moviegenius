...
export default function EntityLinkedText({
  ...
  useEffect(() => {
    // ENABLED: Movie linking for **TITLE** (Year) patterns
    if (!text || linkingStyle === 'off' || !linkMovies) {
      setLinkedText(text);
      return;
    }

    console.log('🔗 EntityLinkedText ENABLED - Processing text for movie linking');
    ...
    const processText = async () => {
      try {
        ...
        while ((match = boldPattern.exec(cleanText)) !== null) {
          ...
          console.log(`🎬 Found bold movie: "${title}" (${year}) at position ${match.index}`);
        ...
        while ((match = legacyPattern.exec(cleanText)) !== null) {
          ...
          const overlaps = matches.some(
            existingMatch =>
              ...
              (existingMatch.start >= match.index &&
                existingMatch.start < match.index + match[0].length)
          );
        ...
        if (isMounted) {
          if (matches.length > 0) {
            ...
            for (const movieMatch of matches.reverse()) {
              ...
              processedText =
                processedText.slice(0, movieMatch.start) +
                link +
                processedText.slice(movieMatch.end);
              linksCreated++;
              console.log(
                `✅ Linked (${movieMatch.type}): "${movieMatch.title}" (${movieMatch.year}) -> TMDB search`
              );
            }
            ...
            console.log(
              `🔗 FINAL RESULT: Created ${linksCreated} movie links: ${boldCount} bold format, ${legacyCount} legacy format`
            );
  ...
  return (
    <span className={className} style={style} dangerouslySetInnerHTML={{ __html: linkedText }} />
  );
}
