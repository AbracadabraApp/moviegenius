  ...
  async batchDatabaseOperation(operation, data, options = {}) {
    ...
    try {
      ...
      for (let i = 0; i < data.length; i += batchSize) {
        ...
        switch (dbOperation) {
          case 'upsert':
            batchResult = await this.supabase.from(table).upsert(batch).select();
            break;
          case 'insert':
            batchResult = await this.supabase.from(table).insert(batch).select();
            break;
          case 'update':
            // For updates, batch data should include where conditions
            batchResult = await this.supabase.from(table).update(batch.data).match(batch.where);
            break;
          default:
