...
/**
 * Enhanced proper name detection implementing documented rules
 */
export function underlineProperNames(text) {
  ...
  const nameRegex = /\b[A-ZÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ×ØÙÚÛÜÝÞßàáâãäåæçèéêëìíîïðñòóôõö÷øùúûüýþÿ][a-zàáâãäåæçèéêëìíîïðñòóôõö÷øùúûüýþÿ]+(?:[-\s][A-ZÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ×ØÙÚÛÜÝÞßàáâãäåæçèéêëìíîïðñòóôõö÷øùúûüýþÿ][a-zàáâãäåæçèéêëìíîïðñòóôõö÷øùúûüýþÿ]*)*(?:\s+(?:Jr\.?|Sr\.?|III|IV))?/g;
  ...
  while ((match = nameRegex.exec(text)) !== null) {
    ...
    // Apply exclusion rules first (highest priority)
    if (shouldExclude(candidate, text, matchStart, matchEnd)) {
      continue;
    }
    
    // Check if it's a proper name we should underline
    const nameResult = isProperName(candidate, text, matchStart, matchEnd);
    if (nameResult.isName) {
      ...
      // Add underlined name (keeping original visual style)
      parts.push(<u key={matchStart}>{candidate}</u>);
      lastIndex = matchEnd;
    }
  }
  
  // Handle quoted movie titles specially
  const quotedTitleRegex = /"([^"]+)"/g;
  let quotedMatch;
  while ((quotedMatch = quotedTitleRegex.exec(text)) !== null) {
    const title = quotedMatch[1];
    const matchStart = quotedMatch.index;
    const matchEnd = quotedMatch.index + quotedMatch[0].length;
    ...
    // Add underlined quoted title (including quotes)
    parts.push(<u key={matchStart}>&quot;{title}&quot;</u>);
    lastIndex = matchEnd;
  }
