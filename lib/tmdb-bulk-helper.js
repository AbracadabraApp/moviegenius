  ...
  async fetchCompleteMovieData(movies) {
    ...
    movies.forEach((movie, index) => {
      ...
      if (movie.tmdb_id) {
        ...
        requests.push(
          this.bulkRequests.searchMovie(`${baseId}_search`, movie.title, movie.year)
        );
      }
    });
...
export async function enhanceMediaCards(mediaCards) {
  ...
  const needsSearch = mediaCards.filter(card => 
    !card.tmdb_id && card.title && card.year
  );
  
  const needsStreaming = mediaCards.filter(card => 
    card.tmdb_id && (!card.initialStreaming || card.initialStreaming === 'TBD')
  );
  
  const needsPosters = mediaCards.filter(card =>
    card.tmdb_id && (!card.initialPoster || card.initialPoster.includes('placeholder'))
  );
  ...
  if (needsSearch.length > 0) {
    promises.push(
      helper.searchMovies(needsSearch.map(card => ({
        title: card.title,
        year: card.year
      })))
    );
  ...
  if (needsStreaming.length > 0) {
    promises.push(
      helper.fetchStreamingData(needsStreaming.map(card => card.tmdb_id))
    );
  }
