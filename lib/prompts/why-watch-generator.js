...
- FOR NO RECOMMENDATIONS: Format as "Problem 1," "Problem 2," "Try <link>Sophisticated Alternative</link> - compelling reason why (8-15 words)"
  * Choose lesser-known, high-quality films over obvious blockbusters
  * Provide brief but compelling reason WHY the alternative is superior
  * Examples: "Terrible dialogue," "Boring pacing," "Try <link>The Long Good Friday</link> - Hoskins' menacing crime boss elevates British gangster cinema beyond Hollywood clichés"
  * Examples: "Bad acting," "Weak plot," "Try <link>Dead Ringers</link> - Cronenberg's psychological horror masterpiece explores identity obsession with surgical precision"
...
export function validateWhyWatchResponse(response) {
  ...
  if (!Array.isArray(reasons) || reasons.length !== 3) {
    ...
    reasons.forEach((reason, index) => {
      ...
      // Allow longer 3rd reason for NO recommendations (8-15 words)
      if (recommendation === 'NO' && index === 2) {
        if (wordCount < 8 || wordCount > 15) {
          errors.push(`NO recommendation reason 3 has ${wordCount} words (should be 8-15): "${reason}"`);
        }
      } else {
