...
class AutoRollbackSystem {
  ...
  async establishBaselines() {
    ...
    try {
      ...
      if (recentResponseTimes.length > 0) {
        const avgResponseTime = recentResponseTimes.reduce((sum, t) => sum + t.value, 0) / recentResponseTimes.length;
  ...
  async performHealthCheck() {
    ...
    try {
      ...
      this.safetyMonitor.recordMetric('auto_rollback_health_check', Date.now(), {
        ...
        system_status: issues.critical.length > 0 ? 'critical' : issues.warning.length > 0 ? 'warning' : 'healthy'
      });
  ...
  async checkPerformanceHealth() {
    ...
    try {
      ...
      if (recentResponseTimes.length > 0) {
        ...
        if (avgResponseTime > baselineResponseTime * (1 + this.criticalThresholds.responseTimeDegradation)) {
          issues.push({
            type: 'response_time_degradation',
            severity: 'critical',
            current: avgResponseTime,
            baseline: baselineResponseTime,
            degradation: ((avgResponseTime - baselineResponseTime) / baselineResponseTime * 100).toFixed(1) + '%',
      ...
      const recentTotal = this.getRecentMetrics('mediacard_cache_hit', 300000).length + recentErrors.length;
  ...
  async checkFeatureHealth() {
    ...
    try {
      ...
      if (this.demoConfig.PREDICTIVE.enabled) {
        ...
        if (successRate < 70) { // Below 70% success rate
          issues.push({
  ...
  async triggerEmergencyRollback(criticalIssues) {
    ...
    console.log('Critical issues:', criticalIssues.map(i => i.message));
    ...
    console.log(`🛡️ Emergency rollback complete: ${this.rollbackState.featuresRolledBack.join(', ')}`);
  }
    ...
    try {
      ...
      if (issues.critical.length === 0) {
        ...
        if (rollbackAge > recoveryPeriod) {
          ...
          console.log(`🛡️ System stable but waiting for recovery period (${Math.round((recoveryPeriod - rollbackAge) / 1000)}s remaining)`);
        }
      } else {
  ...
  checkEmergencyConditions() {
    ...
    try {
      ...
      if (currentMemory > memoryLimit * 0.98) { // 98% memory usage
  ...
  getStatus() {
    return {
      ...
      last_health_check: this.getRecentMetrics('auto_rollback_health_check', 300000).length > 0 ? 
        this.getRecentMetrics('auto_rollback_health_check', 300000).slice(-1)[0].timestamp : null
    };
  }
