...
const BATCH_CONFIG = {
  ...
  TARGET_COST_PER_HOUR: 10.0, // Maximum spend per hour
};
...
class NuclearBatchGenerator {
  ...
  async filterExistingAnalysis(movieIds) {
    ...
    const { data: moviesNeedingAnalysis } = await supabase
      ...
      .in(
        'id',
        movieIds.filter(id => !existingMovieIds.has(id))
      );
  ...
  async processBatchesConcurrently(batches, maxConcurrency) {
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      const batchPromise = this.processBatch(batches[i], i + 1).then(result => {
  ...
  async processBatch(movies, batchNumber) {
    ...
    try {
      ...
      // Submit batch to Claude (beta API)
    ...
    while (Date.now() - startTime < maxWaitTime) {
      try {
        ...
        if (batch.processing_status === 'completed') {
          console.log(
            `✅ Batch ${batchNumber} completed in ${Math.round((Date.now() - startTime) / 1000)}s`
          );
          return batch;
        }
        ...
        if (batch.processing_status === 'failed') {
          throw new Error(
            `Batch ${batchNumber} failed: ${batch.errors?.[0]?.message || 'Unknown error'}`
          );
        }

        console.log(
          `⏳ Batch ${batchNumber} status: ${batch.processing_status}, waiting ${pollInterval}ms...`
        );
      ...
      for (const result of batchResults) {
        try {
          ...
          if (result.result.type === 'succeeded') {
            ...
            const costEstimate =
              ((usage.input_tokens * 3) / 1000000 + (usage.output_tokens * 15) / 1000000) *
