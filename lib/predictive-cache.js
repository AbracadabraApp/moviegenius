...
class PredictiveCacheManager {
  ...
  async getPredictiveResponse(question) {
    ...
    if (exactMatch) {
      console.log(`🚀 Predictive cache HIT: ${question}`);
      return exactMatch;
    }
    ...
    for (const pattern of QUESTION_PATTERNS) {
      ...
      if (match) {
        ...
        if (patternMatch) {
          console.log(`🎯 Pattern cache HIT: ${question} -> ${patternKey}`);
          return patternMatch;
        }
      }
    }
  ...
  async warmPredictiveCache() {
    if (this.isWarming) {
      console.log('⏳ Predictive cache warming already in progress');
      return;
    }
    ...
    try {
      for (let i = 0; i < COMMON_QUESTIONS.length; i++) {
        ...
        try {
          ...
          console.log(`✅ Cached predictive response for: "${question}"`);
  ...
  async analyzeForCaching(question) {
    ...
    if (newCount >= 3) {
      console.log(`🔥 Popular question detected (${newCount}x): "${question}"`);
