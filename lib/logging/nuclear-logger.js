...
export class NuclearLogger {
  ...
  static logCapture(tmdbId, movieTitle, fileSize, processingTime = null) {
    if (!initializeLogger()) {
      ...
      console.log(`📦 Nuclear captured: ${movieTitle} (${tmdbId}) - ${Math.round(fileSize / 1024)}KB`);
      return;
    }
  ...
  static logCaptureFailure(tmdbId, movieTitle, error, errorType = 'unknown') {
    ...
    console.warn(`❌ Nuclear capture failed: ${movieTitle} (${tmdbId}) - ${error.message || error}`);
  }
  ...
  static logNuclearServe(tmdbId, movieTitle, loadTime = null) {
    ...
    writeToLog('INFO', message, data);
    console.log(`⚡ Nuclear served: ${movieTitle} (${tmdbId})${loadTime ? ` - ${loadTime}ms` : ''}`);
  }
  ...
  static getErrorSummary() {
    ...
    return {
      ...
      errorRate: Math.round(((metrics.totalAttempts - metrics.totalCaptures) / metrics.totalAttempts) * 100)
    };
  }
  ...
  static generateDailyReport() {
    ...
    return {
      ...
      summary: {
        ...
        successRate: todayStats.attempts > 0 ? Math.round((todayStats.captures / todayStats.attempts) * 100) : 0,
