...
class DatabaseOptimizer {
  ...
  async lookupMovie(title, year) {
    return await this.measureQueryPerformance('movie_title_year_lookup', async () => {
  ...
  async lookupMovieByTmdbId(tmdbId) {
    return await this.measureQueryPerformance('movie_tmdb_lookup', async () => {
      const { data, error } = await this.supabase
        .from('movies')
        .select('*')
        .eq('tmdb_id', tmdbId)
  ...
  async searchMovies(query, limit = 20) {
    return await this.measureQueryPerformance('movie_fuzzy_search', async () => {
      const { data, error } = await this.supabase
        .from('movies')
  ...
  async getMovieAnalysis(movieId, analysisType = 'page_analysis') {
    return await this.measureQueryPerformance('movie_analysis_lookup', async () => {
      const { data, error } = await this.supabase
        .from('movie_analyses')
  ...
  getQueryStats() {
    ...
    for (const [queryName, queryStats] of this.queryStats.entries()) {
      stats[queryName] = {
        ...queryStats,
        slowQueryPercentage: ((queryStats.slowQueries / queryStats.totalExecutions) * 100).toFixed(1) + '%'
      };
    }
  ...
  async optimizeDatabase() {
    ...
    try {
      ...
      const successfulCritical = criticalResults.filter(r => r.status === 'created' || r.status === 'exists').length;
      let textResults = [];
      ...
      const summary = {
        ...
        total_indexes_created: [...criticalResults, ...textResults].filter(r => r.status === 'created').length,
