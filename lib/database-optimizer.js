...
class DatabaseOptimizer {
    ...
    return await this.measureQueryPerformance(
      ...
      async () => {
        ...
        const result = await pool.query(
          'SELECT * FROM movies WHERE title = $1 AND year = $2',
          [title, year]
        );
  ...
  async getMovieAnalysis(movieId, analysisType = 'page_analysis') {
    return await this.measureQueryPerformance(
      ...
      async () => {
        const pool = getPool();
        const result = await pool.query(
          'SELECT claude_response FROM movie_analyses WHERE movie_id = $1 AND analysis_type = $2',
          [movieId, analysisType]
        );
  ...
  async cleanupExpiredCache() {
    return await this.measureQueryPerformance('cache_cleanup', async () => {
      const pool = getPool();
      const result = await pool.query(
        'DELETE FROM query_cache WHERE expires_at < NOW() RETURNING *'
      );
