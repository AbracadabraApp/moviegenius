 ...
 * - JSON format with keyElements object (new)
 * - Text format with KEY_CONTRIBUTORS: line (legacy)
 */
function extractContributorsFromKeyElements(rawContent) {
    ...
    if (parsed.keyElements) {
      ...
      if (Array.isArray(keyElements.writers)) {
        keyElements.writers.forEach(name => {
          if (name && !contributors.find(c => c.name === name)) {
            contributors.push({ name, role: 'writer' });
          }
        });
      }
      ...
      if (Array.isArray(keyElements.stars)) {
        keyElements.stars.forEach(name => {
          if (name && !contributors.find(c => c.name === name)) {
            contributors.push({ name, role: 'star' });
          }
        });
      }
      ...
      console.log(`📋 Extracted ${contributors.length} contributors from JSON keyElements:`, contributors.map(c => c.name));
      return contributors;
    }
  } catch (e) {
    // Not JSON or no keyElements, try text format
  }

  // Fallback: Format 2 - KEY_CONTRIBUTORS: Director: Name, Writer: Name, Star: Name1, Name2
  ...
  if (keyElementsMatch) {
    ...
    console.log(`📋 Extracted ${contributors.length} contributors from text KEY_CONTRIBUTORS:`, contributors.map(c => c.name));
  }
