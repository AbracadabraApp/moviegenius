...
async function lookupMovieInDB(title, year = null, client = null) {
  ...
  try {
    ...
    // For title-only search, we'll need a custom query since MovieService doesn't have fuzzy search
    const shouldReleaseClient = !client;
    const dbClient = client || getPool();
    
    try {
      const titleQuery = 'SELECT tmdb_id, title, year, poster_url FROM movies WHERE LOWER(title) = LOWER($1) ORDER BY year DESC LIMIT 1';
      const titleResult = await dbClient.query(titleQuery, [title]);
...
async function processAnalysisContent(content, currentMovieTitle = '', context = '', rawContent = '', options = {}) {
  ...
  if (processContributors && rawContent && isNode) {
    const contributors = extractContributorsFromKeyElements(rawContent);
