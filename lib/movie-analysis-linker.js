...
async function getStaticMoviePages(limit = 20) {
  try {
    ...
    const files = fs.readdirSync(nuclearDir)
      ...
      .slice(0, limit)
      .map(f => {
        const tmdbId = parseInt(f.replace('.json', ''));
        return { tmdbId, filename: f };
      });
...
export async function testMovieAnalysisLinking(content, currentMovie = '') {
  ...
  mentions.forEach((mention, index) => {
    console.log(`${index + 1}. ${mention.type}: "${mention.title}"${mention.year ? ` (${mention.year})` : ''}`);
  });
...
export { extractMovieMentions, lookupMovieInDB, processAnalysisContent };