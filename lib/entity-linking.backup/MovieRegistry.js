...
export class MovieRegistry {
  ...
  async detectTitleWithYearMatches(text, context) {
    ...
    for (const pattern of yearPatterns) {
      ...
      while ((match = pattern.exec(text)) !== null) {
        ...
        // Skip if title is too short or contains non-title words
        if (titleCandidate.length < 2 || this.containsNonTitleWords(titleCandidate)) {
          continue;
        }

        const movie = await this.findMovie(titleCandidate, year, context);
        if (movie) {
          matches.push({
            movie,
            text: match[0].trim(),
  ...
  containsNonTitleWords(text) {
    ...
    return words.some(word => nonTitleWords.includes(word));
  }
  ...
  async linkMovies(text, detectedMovies) {
    ...
    for (const movieMatch of sortedMovies) {
      ...
      const link = `<a href="/movie/${movie.slug}" class="movie-link" data-movie-id="${movie.id}" data-confidence="${movieMatch.confidence}">${movieMatch.text}</a>`;

      linkedText = linkedText.slice(0, movieMatch.start) + link + linkedText.slice(movieMatch.end);
      linksCreated++;
    }
