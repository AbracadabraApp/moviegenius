...
async function loadFromDatabase(tmdbId) {
  ...
  const [tmdbRes, analysisRes, streamingRes] = await Promise.all([
    ...
    fetch(`/api/movie-data?id=${tmdbId}`)
  ]);
 ...
 * Preserves original format for MovieAnalysisWithEntities compatibility
 */
function parseAnalysisResponse(analysisData) {
  ...
  // PRESERVE RAW FORMAT for MovieAnalysisWithEntities
  // This component expects: { claude_response: { raw_content }, entity_linking_data }
  //
  // CRITICAL: analysisData.analysis contains HTML with entity links (from processed_content in DB)
  // Put it in BOTH raw_content and processed_content so component renders HTML correctly
  // (Component tries JSON.parse on raw_content, fails, then renders as HTML - this is correct!)
  const rawFormat = {
    claude_response: {
      raw_content: analysisData.analysis || '',      // HTML with links (not JSON!)
      processed_content: analysisData.analysis || '' // Same - HTML with links
    },
  ...
  // Also provide parsed sections for simpler components (backward compat)
  ...
  try {
    ...
    featuredMovies = analysisData.entityData?.featuredMovies
  ...
  return {
    ...
    moreIdeas: [],
    exploreTopics: [],
    rawData: rawFormat  // Preserve original format for MovieAnalysisWithEntities
  };
}
