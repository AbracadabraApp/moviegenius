...
class AutonomousNuclearSystem {
  ...
  async getNextBatch() {
    try {
      ...
      const nuclearCandidates = candidatesResult.rows;
      if (!nuclearCandidates || nuclearCandidates.length === 0) return [];
      ...
      const analysesResult = await pool.query(`
        ...
        FROM movie_analyses 
        WHERE analysis_type = 'page_analysis' AND movie_id = ANY($1)
      `, [nuclearIds]);
      
      const existingAnalyses = analysesResult.rows;
  ...
  async saveAnalysis(movie, analysis, usage, cost) {
    ...
    const pool = getPool();
    await pool.query(`
      INSERT INTO movie_analyses (movie_id, analysis_type, claude_response, query_text, created_at, updated_at)
      VALUES ($1, $2, $3, $4, NOW(), NOW())
    `, [
      movie.id,
      'page_analysis',
      JSON.stringify(analysisData),
      `Autonomous nuclear conversion for ${movie.title} (${movie.year})`
    ]);
  }
