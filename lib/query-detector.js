...
class QueryDetector {
    ...
    return await this.cache.cacheAside(
      ...
      async () => {
        try {
          ...
          const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/genius-topics`);
          if (!response.ok) {
  ...
  extractTopicKeywords(query) {
    ...
    const stopWords = ['what', 'is', 'are', 'the', 'about', 'tell', 'me', 'how', 'why', 'when', 'where'];
  ...
  calculateGeniusTopicScore(queryKeywords, geniusItem) {
    let score = 0;
    const itemText = `${geniusItem.title} ${geniusItem.subtitle || ''} ${geniusItem.description || ''}`.toLowerCase();
