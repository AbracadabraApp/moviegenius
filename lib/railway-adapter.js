...
// Railway adapter - browser safe version
...
// Browser-safe pool getter
async function getPoolSafe() {
  if (isBrowser) {
    throw new Error('Database operations not available in browser - use API endpoints');
  }
  
  // Dynamic import to avoid loading pg in browser
  const { getPool } = await import('./railway-db.js');
  return getPool();
}
