...
export async function getCachedGeniusConfig() {
  ...
  if (!cachedConfig || isExpired) {
    ...
    try {
      ...
      // TODO: Convert callers to async when ES modules are fully adopted
      if (typeof window === 'undefined') {
        // Server-side: use dynamic import to avoid bundling
        const fs = await import('fs');
        const path = await import('path');
        const configPath = path.resolve(process.cwd(), 'data/genius-config.json');
        const configData = fs.readFileSync(configPath, 'utf8');
        cachedConfig = JSON.parse(configData);
      } else {
        // Client-side: return fallback, will load dynamically later
        cachedConfig = {
          themes: {},
          series: {},
          episodes: {},
 ...
 */
export async function getCachedSeriesData(themeId, seriesId) {
  const config = await getCachedGeniusConfig();
 ...
 */
export async function getCachedOtherSeries(currentThemeId, currentSeriesId) {
  const config = await getCachedGeniusConfig();
  const allSeries = [];
 ...
 */
export async function getCachedOtherEpisodes(themeId, seriesId, currentEpisodeId) {
  const config = await getCachedGeniusConfig();
