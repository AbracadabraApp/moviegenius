 ...
 */
export function getCachedGeniusConfig() {
  const now = Date.now();
  ...
  if (!cachedConfig || isExpired) {
    console.log('🔄 Loading genius config into cache...');
    
    try {
      // Only perform the expensive require() when cache is invalid
      cachedConfig = require('../data/genius-config.json');
      cacheTimestamp = now;
      
      console.log(`✅ Genius config cached successfully (${Object.keys(cachedConfig.themes).length} themes)`);
    } catch (error) {
...
export function clearGeniusConfigCache() {
  cachedConfig = null;
  cacheTimestamp = null;
  console.log('🗑️ Genius config cache cleared');
}
