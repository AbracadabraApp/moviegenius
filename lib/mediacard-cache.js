...
class MediaCardCache {
  ...
  async initializeDemoConfig() {
    try {
      ...
      if (this.demoConfig.ENABLED) {
        ...
        console.log(
          `   • TTL strategy: ${this.demoConfig.CACHING.mediaCardTTL === 0 ? 'FOREVER' : this.demoConfig.CACHING.mediaCardTTL + 's'}`
        );
  ...
  async get(key) {
    ...
    if (this.memoryCache.has(key)) {
      ...
      const isExpired =
        this.memoryCacheTTL > 0 && Date.now() - cached.timestamp > this.memoryCacheTTL;
    ...
    if (this.demoConfig.ENABLED && this.safetyMonitor) {
      ...
      if (hitRate < this.demoConfig.CACHING.hitRateThreshold * 100) {
        console.warn(
          `⚠️ MediaCard cache hit rate dropping: ${hitRate.toFixed(1)}% (target: ${this.demoConfig.CACHING.hitRateThreshold * 100}%)`
        );
      }
    }
  ...
  async batchCacheMovieData(movies) {
    const startTime = Date.now();
    const promises = movies.map(movie => this.cacheMovieData(movie.title, movie.year, movie.data));
  ...
  getCacheStats() {
    ...
    return {
      ...
      memoryCacheUtilization:
        ((this.memoryCache.size / this.memoryCacheSize) * 100).toFixed(1) + '%',
      // Demo mode metrics
      ...
      requestsPerSecond:
        this.cacheStats.totalRequests > 0
          ? (this.cacheStats.totalRequests / (uptime / 1000)).toFixed(2)
          : '0',
  ...
  checkMemoryUsage() {
    ...
    if (memoryUsageMB > memoryThreshold) {
      console.warn(
        `⚠️ MediaCard cache memory usage high: ${memoryUsageMB.toFixed(1)}MB (limit: ${memoryThreshold}MB)`
      );
