...
export function normalizeMovie(movie) {
  ...
  if (typeof normalizedMovie.tmdb_id === 'string' && normalizedMovie.tmdb_id.startsWith('tt')) {
    ...
    if (tmdbId) {
      console.log(`🔄 Converting IMDb ID ${normalizedMovie.tmdb_id} to TMDB ID ${tmdbId} for "${movie.title}"`);
  ...
  if (!normalizedMovie.tmdb_id || normalizedMovie.tmdb_id === null) {
    const lookup = MOVIES_NEEDING_TMDB_LOOKUP.find(m => 
...
export function normalizeEpisodeContent(episodeData) {
  ...
  if (normalizedData.content.moreIdeas?.movies) {
    normalizedData.content.moreIdeas.movies = normalizedData.content.moreIdeas.movies.map(normalizeMovie);
  }
