...
export async function validateEpisodeData(episodeData) {
      ...
      if (section.type === 'movies' && section.movies) {
        ...
        for (const movie of section.movies) {
          ...
          if (validation.errors.length > 0) {
            warnings.push(`Movie validation errors for "${movie.title}": ${validation.errors.join(', ')}`);
          }
          
          if (validation.warnings.length > 0) {
            console.log(`Movie validation warnings for "${movie.title}": ${validation.warnings.join(', ')}`);
          }
  ...
  if (episodeData.content?.moreIdeas?.movies) {
    ...
    for (const movie of episodeData.content.moreIdeas.movies) {
      ...
      if (validation.errors.length > 0) {
        warnings.push(`MoreIdeas movie validation errors for "${movie.title}": ${validation.errors.join(', ')}`);
      }
