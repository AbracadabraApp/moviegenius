...
class PerformanceMonitor {
  ...
  trackMetric(metric, value, context = {}) {
    ...
    if (this.baselines.has(baselineKey)) {
      ...
      if (this.isDevelopment) {
        console.log(`📈 ${metric}: ${value} (${improvement > 0 ? '+' : ''}${improvement.toFixed(1)}% vs baseline)`);
      }
  ...
  validateImprovement(metric, improvement, context) {
    ...
    if (expected && improvement < expected * 0.8) { // 80% of expected
      console.warn(`⚠️ Performance warning: ${metric} improvement (${improvement.toFixed(1)}%) below expected (${expected}%)`);
    } else if (expected && improvement >= expected) {
      console.log(`✅ Performance target met: ${metric} improved ${improvement.toFixed(1)}% (target: ${expected}%)`);
    }
  }
  ...
  async timeFunction(name, fn, context = {}) {
    ...
    try {
      ...
      const memoryDelta = endMemory && startMemory ? 
        endMemory.heapUsed - startMemory.heapUsed : null;
  ...
  trackCachePerformance(operation, key, hit, responseTime, context = {}) {
    ...
    if (hitRate < this.thresholds.cacheHitRate) {
      console.warn(`📊 Cache hit rate below threshold: ${hitRate.toFixed(1)}% (target: ${this.thresholds.cacheHitRate}%)`);
    }
  }
  ...
  trackAPICost(service, operation, inputTokens = 0, outputTokens = 0, cached = false) {
    ...
    if (this.isDevelopment && !cached) {
      console.log(`💰 API Cost: ${service} ${operation} = $${cost.toFixed(4)} (${inputTokens + outputTokens} tokens)`);
    }
  }
  ...
  initCoreWebVitals() {
    ...
    console.debug('Performance Monitor: Core Web Vitals tracking disabled (web-vitals package not installed)');
    return;
   ...
   */
  generateReport(timeWindow = 3600000) { // Default 1 hour
    ...
    for (const [metric, entries] of this.metrics.entries()) {
      ...
      if (this.baselines.has(baselineKey)) {
        const baseline = this.baselines.get(baselineKey);
        const improvement = ((baseline.value - report.metrics[metric].average) / baseline.value) * 100;
    ...
    if (this.calculateCacheHitRate() < this.thresholds.cacheHitRate) {
      report.recommendations.push('Cache hit rate below target - consider cache warming or TTL adjustments');
    }
  ...
  detectPerformanceRegression(metric, threshold = 20) {
    ...
    if (this.baselines.has(baselineKey)) {
      ...
      if (regression > threshold) {
        console.error(`🚨 Performance regression detected: ${metric} increased ${regression.toFixed(1)}% above baseline`);
        return true;
      }
    }
