...
export async function isFeatureEnabled(flagName) {
  ...
  if (flag.environments && flag.environments[environment] !== undefined) {
    ...
    console.log(
      `Feature '${flagName}' ${enabled ? 'enabled' : 'disabled'} by environment (${environment})`
    );
    return enabled;
  }
  ...
  if (rolloutPercentage === 'auto' && flag.rolloutTestName) {
    try {
      const { getCurrentRolloutPercentage } = await import('./rolloutConfig.js');
  ...
  console.log(
    `Feature '${flagName}' ${enabled ? 'enabled' : 'disabled'} for bucket ${userBucket} (rollout: ${rolloutPercentage}%)`
  );
  return enabled;
}
...
// Export the override-aware version for development (ES modules don't use conditional exports the same way)
