...
const FEATURE_FLAGS = {
  ...
  HEADER_B_VARIANT: {
    enabled: false,
    rolloutPercentage: 0, // 0-100, percentage of users to show B variant
    environments: {
...
export function isFeatureEnabled(flagName) {
  ...
  console.log(`Feature '${flagName}' ${enabled ? 'enabled' : 'disabled'} for bucket ${userBucket} (rollout: ${flag.rolloutPercentage}%)`);
  return enabled;
}
