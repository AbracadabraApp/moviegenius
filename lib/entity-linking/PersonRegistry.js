...
export class PersonRegistry {
  ...
  async _doLoad() {
    try {
      ...
      console.log(
        `PersonRegistry loaded: ${people.length} people, ${variations.length} variations`
      );
    } catch (error) {
  ...
  async findPerson(name, context = 'general') {
    ...
    const slug = this.nameIndex.get(normalizedName);
    if (slug) {
  ...
  async linkPeople(text, detectedPeople) {
    ...
    for (const personMatch of sortedPeople) {
      ...
      linkedText =
        linkedText.slice(0, personMatch.start) + link + linkedText.slice(personMatch.end);
      linksCreated++;
    }
