...
export const DEMO_CONFIG = {
  ...
  CACHING: {
    ...
    mediaCardTTL:
      process.env.DEMO_MEDIACARD_TTL === 'forever'
        ? 0
        : parseInt(process.env.DEMO_MEDIACARD_TTL) || 86400,
...
export class DemoSafetyMonitor {
  ...
  checkSafetyThresholds(metric, currentValue) {
    ...
    const baseline = measurements.slice(0, 10).reduce((sum, m) => sum + m.value, 0) / 10;
