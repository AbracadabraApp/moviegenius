...
class RedisService {
  constructor() {
    ...
    this.TTL = {
      CLAUDE_ANALYSIS: 24 * 60 * 60,      // 24 hours - Claude responses are stable
      TMDB_DATA: 7 * 24 * 60 * 60,        // 7 days - Movie metadata rarely changes
      PERSON_DATA: 7 * 24 * 60 * 60,      // 7 days - Person data is stable
      MOVIE_LOOKUPS: 24 * 60 * 60,        // 24 hours - Movie queries
      DATABASE_QUERIES: 60 * 60,          // 1 hour - Database results
      SEARCH_RESULTS: 30 * 60,            // 30 minutes - Search queries
      TAG_CLOUD: 6 * 60 * 60,             // 6 hours - Tag cloud data
      STREAMING_DATA: 12 * 60 * 60        // 12 hours - Streaming availability
    };
  ...
  generateKey(type, identifier, version = 'v1') {
    const safeId = typeof identifier === 'object' 
  ...
  async set(key, data, ttlSeconds = this.TTL.DATABASE_QUERIES) {
    ...
    try {
      ...
      console.log(`💾 Cache SET: ${key} (${responseTime}ms, TTL: ${ttlSeconds}s, Size: ${serialized.length} bytes)`);
      return true;
    } catch (error) {
