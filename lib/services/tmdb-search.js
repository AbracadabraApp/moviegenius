...
// Simplified TMDB authentication - API key only
function getTMDBApiKey() {
  // Use same API key for both development and production
...
export async function searchTMDB(query) {
  ...
  try {
    // Build TMDB search URL using API key method
    ...
    const url = `${TMDB_BASE_URL}/search/movie?${searchParams}`;
    const headers = {
...
export async function getTMDBMovieDetails(tmdbId) {
  ...
  try {
    // Use simple API key method - consistent with other endpoints
    const url = `${TMDB_BASE_URL}/movie/${tmdbId}?api_key=${getTMDBApiKey()}`;
    const headers = {
      'Accept': 'application/json',
    };
