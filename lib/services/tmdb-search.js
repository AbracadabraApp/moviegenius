...
import { parseSearchQuery } from '../utils/search-utils.js';

// TMDB authentication - supports both Bearer token and API key
function getTMDBAuthConfig() {
  // First try Bearer token (production preference) - TMDB v4 tokens are JWTs
  ...
  // Fallback to API key method - prioritize server-side key over public key
  ...
  // Last resort - check public key (but validate it's not placeholder)
  const publicApiKey = process.env.NEXT_PUBLIC_TMDB_API_KEY;
  if (publicApiKey && publicApiKey !== 'placeholder' && !publicApiKey.startsWith('placehol') && publicApiKey.length > 10) {
    return {
      method: 'apikey',
      token: publicApiKey,
      headers: {}
  ...
  console.error('❌ TMDB authentication failed: No valid API key or Bearer token found');
  return null;
}
...
export async function searchTMDB(query) {
  ...
  try {
    // Build TMDB search URL with proper authentication
...
export async function getTMDBMovieDetails(tmdbId) {
  ...
  try {
    ...
    if (authConfig.method === 'bearer') {
      ...
      // Use API key method
      url = `${TMDB_BASE_URL}/movie/${tmdbId}?api_key=${authConfig.token}`;
      headers = { 'Accept': 'application/json' };
    }
    ...
    if (!response.ok) {
      if (response.status === 401) {
        const rateLimitRemaining = response.headers.get('X-RateLimit-Remaining');
        console.error('TMDB details 401 Unauthorized:', {
          authMethod: authConfig.method,
          rateLimitRemaining,
          possibleCauses: ['Invalid/expired token', 'Rate limit exceeded', 'Wrong scope']
        });
