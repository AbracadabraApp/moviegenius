...
// Initialize Bearer token lazily with validation and fallback
function getTMDBBearerToken() {
  const bearerToken = process.env.TMDB_BEARER_TOKEN;
  
  // Validate Bearer token - should be a proper JWT (3 parts separated by dots) 
  if (bearerToken && bearerToken.split('.').length === 3) {
    return bearerToken;
  }
  
  console.warn('🔧 TMDB_BEARER_TOKEN invalid or missing, falling back to API key');
  return null;
}

// Get API key for query parameter method (fallback)
...
export async function getTMDBMovieDetails(tmdbId) {
  ...
  try {
    ...
    if (bearerToken) {
      ...
      // Fall back to API key method
      url = `${TMDB_BASE_URL}/movie/${tmdbId}?api_key=${getTMDBApiKey()}`;
      headers = {
        'Accept': 'application/json',
      };
    }
