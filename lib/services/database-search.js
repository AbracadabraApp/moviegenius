...
export async function createBasicMovieEntry(tmdbMovie) {
  try {
    ...
    if (error) {
      // If duplicate TMDB ID, try to find existing movie
      if (error.code === '23505' || error.message?.includes('duplicate') || error.message?.includes('unique')) {
        console.log(`🔍 Movie already exists in database, fetching existing entry for TMDB ${tmdbMovie.id}`);
        
        const { data: existingMovie, error: fetchError } = await getSupabaseClient()
          .from('movies')
          .select('*')
          .eq('tmdb_id', tmdbMovie.id)
          .single();
          
        if (existingMovie && !fetchError) {
          console.log(`✅ Found existing movie: "${existingMovie.title}" (${existingMovie.year})`);
