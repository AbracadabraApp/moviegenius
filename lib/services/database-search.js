...
export async function searchFuzzyMatch(title, year) {
  try {
    ...
    console.log(`🔍 Database fuzzy match: "${title}" (${year}) -> "${bestMatch.title}" (${bestMatch.year})`);
    return bestMatch;
...
export async function createBasicMovieEntry(tmdbMovie) {
  try {
    ...
    console.log(`💾 Created database entry: "${tmdbMovie.title}" (${releaseYear}) -> TMDB ${tmdbMovie.id}`);
    return newMovie;
