...
async function evaluateForPromotion(tmdbId) {
  try {
    ...
    const shouldPromote = viewCount >= criteria.minViews && 
                         uniqueSources >= criteria.minSources;
    
    console.log(`🎯 Promotion evaluation for TMDB ${tmdbId}: ${viewCount} views, ${uniqueSources} sources -> ${shouldPromote ? 'PROMOTE' : 'WAIT'}`);
