...
export class AnalysisService {
  ...
  static async getOrGenerate(movie, options = {}) {
    console.log(`🎯 getOrGenerate called for movie: ${movie.title} (ID: ${movie.id}, TMDB: ${movie.tmdb_id})`);
  ...
  static async getExisting(movieId) {
    console.log(`🗄️ getExisting: Querying database for movie_id=${movieId}, analysis_type=${ANALYSIS_CONFIG.DATABASE.ANALYSIS_TYPE}`);
    ...
    try {
      ...
      if (analysis) {
        ...
        console.log(`🗄️ Analysis content length:`, analysis.claude_response?.raw_content?.length || 0);
      }
  ...
  static parseMovieLine(line) {
    ...
    console.log('🎬 Parsed movie for enhancement:', `${movieObj.title} (${movieObj.year}) - needsTmdbLookup: ${movieObj.needsTmdbLookup}`);
    return movieObj;
  }
  ...
  static async parseAnalysisWithTmdbLookup(responseText) {
    ...
    if (moviesNeedingLookup.length > 0) {
      console.log('🚀 Calling enhanceMoviesWithTmdbIds for:', moviesNeedingLookup.map(m => `${m.title} (${m.year})`));
      await this.enhanceMoviesWithTmdbIds(moviesNeedingLookup);
      console.log('✅ Enhancement complete for:', moviesNeedingLookup.map(m => `${m.title} - TMDB:${m.tmdb_id}`));
    } else {
      ...
      try {
        ...
        if (existingMovie?.tmdb_id) {
          ...
          if (tmdbResult.type === 'single_match') {
            ...
            console.log(`❌ TMDB result type "${tmdbResult.type}" not handled for "${movie.title}"`);
          }
        }
