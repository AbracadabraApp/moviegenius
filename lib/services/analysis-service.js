...
export class AnalysisService {
   ...
   * 🔒 PERFORMANCE CRITICAL: Batch queries to prevent MediaCard API calls
   */
    ...
    for (const movie of movies) {
      try {
        ...
        if (tmdbResult.type === 'single_match') {
          ...
          // 🔒 CRITICAL: Get poster URL from database to prevent MediaCard API calls
          ...
          if (dbMovie?.poster_url) {
            movie.poster_url = dbMovie.poster_url;
            movie.poster = dbMovie.poster_url; // Also set poster field for compatibility
            console.log(`✅ Found poster URL for "${movie.title}"`);
          } else {
            console.log(`⚠️  No poster URL in database for "${movie.title}" TMDB:${movie.tmdb_id}`);
          }
        } else {
