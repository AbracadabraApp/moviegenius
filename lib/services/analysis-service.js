...
export class AnalysisService {
  ...
  static async getOrGenerate(movie, options = {}) {
    try {
      ...
      if (existing && !options.forceRegenerate) {
        return this.parseAnalysis(existing.claude_response.raw_content);
      }
  ...
  static async generate(movie) {
    ...
    const message = await anthropic.messages.create({
      ...
      max_tokens: NUCLEAR_CONFIG.CLAUDE.MAX_TOKENS
    });
    ...
    await supabase
      ...
      .insert({
        movie_id: movie.id,
        analysis_type: NUCLEAR_CONFIG.DATABASE.ANALYSIS_TYPE,
        claude_response: analysisData,
        query_text: `Nuclear analysis for ${movie.title} (${movie.year})`
      });
