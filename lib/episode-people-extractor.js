...
function addPersonToEpisode(peopleList, person, role, movie) {
  ...
  let existingPerson = peopleList.find(p => p.tmdb_id === person.id);
...
export function findPersonInEpisode(name, episodePeople) {
  ...
  let match = episodePeople.allPeople.find(person => 
  ...
  if (lastName.length > 3) { // Avoid matching short words
  ...
  match = episodePeople.allPeople.find(person => 
