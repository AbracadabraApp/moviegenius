...
export class CinematicProfileGenerator {
  ...
  analyzeUserData(userData) {
    ...
    return {
      ...
      personalityType: this.determinePersonalityType(heartedMovies, bookmarkedMovies, selectedPlatforms),
  ...
  generateUserFingerprint(userData) {
    ...
    const heartedTitles = heartedMovies.map(m => m.title || m.id).sort().join(',');
    const bookmarkedTitles = bookmarkedMovies.map(m => m.title || m.id).sort().join(',');
