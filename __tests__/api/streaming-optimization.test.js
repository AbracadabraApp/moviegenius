  ...
  return jest.fn().mockImplementation(() => ({
    messages: {
      create: jest.fn().mockResolvedValue({
        content: [
          {
            text: 'Available on Netflix, Amazon Prime Video for streaming. Also available for rent on Amazon Prime Video, Apple TV, and Google Play Movies.',
          },
        ],
...
describe('Streaming Endpoint Optimization', () => {
  ...
  test('should demonstrate performance improvement potential', () => {
    ...
    const averageResponseTime =
      (cacheHitRate / 100) * cachedResponseTime + ((100 - cacheHitRate) / 100) * baselineApiTime;
