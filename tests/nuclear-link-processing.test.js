...
const { buildMovieLookup, processTextLinks, validateStaticData } = require('../lib/utils/nuclear-link-utils.js');
...
describe('Nuclear Link Processing', () => {
  ...
  describe('buildMovieLookup', () => {
    ...
    test('should handle sections without movies', () => {
      const sections = [
        { type: 'text', content: 'Only text here' }
      ];
  ...
  describe('validateStaticData', () => {
      ...
      const validData = {
        props: {
          ...
          sections: [
            {
              type: 'text',
              content: 'This film influenced <a href="/movie/456" class="movie-title">Other Movie</a> (2001).'
            },
            {
              type: 'movies',
              movies: [
    ...
    test('should detect search-based links', () => {
      ...
      expect(result.issues).toContain('Section 0 has search-based links: href="/search?q=other+movie"');
    });
    ...
    test('should detect unprocessed movie links', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.issues).toContain('Section 0 has unprocessed movie links: **Other Movie** (2001)');
    });
      ...
      const dataWithSelfRef = {
        props: {
          ...
          sections: [
            {
              type: 'text',
              content: '**Test Movie** (2000) is a great film that influenced **Other Movie** (2001).'
            }
          ]
        }
      };
    ...
    test('should detect placeholder posters', () => {
          ...
          sections: [
            {
              type: 'movies',
              movies: [
                { title: 'Other Movie', year: 2001, tmdb_id: 456, poster_url: '/images/placeholder-poster.jpg' }
              ]
            }
          ]
        }
      ...
      expect(result.valid).toBe(false);
      expect(result.issues).toContain('Main movie has placeholder poster: /images/placeholder-poster.jpg');
      expect(result.issues).toContain('Movie 0 in section 0 has placeholder poster: /images/placeholder-poster.jpg');
    });
          ...
          sections: [
            {
              type: 'movies',
              movies: [
                { title: 'Other Movie', year: 2001, tmdb_id: null }
              ]
            }
          ]
        }
  ...
  describe('End-to-End Validation Tests', () => {
      ...
      const testData = {
        props: {
          ...
          sections: [
            {
              type: 'text',
              content: 'This film influenced <a href="/movie/3082" class="movie-title">Modern Times</a> (1936).'
            },
            {
              type: 'movies',
              movies: [
                { title: 'Modern Times', year: 1936, tmdb_id: 3082, poster_url: 'https://image.tmdb.org/t/p/w500/valid.jpg' }
              ]
            }
          ]
        }
    ...
    test('should detect self-referential processed links', () => {
      const dataWithSelfLink = {
        props: {
          ...
          sections: [
            {
              type: 'text',
              content: 'This film <a href="/movie/901" class="movie-title">City Lights</a> (1931) is great.'
            }
          ]
        }
      };
      ...
      expect(result.valid).toBe(false);
      expect(result.issues).toContain('Section 0 has self-referential link: <a href="/movie/901" class="movie-title">City Lights</a>');
    });
  });
});