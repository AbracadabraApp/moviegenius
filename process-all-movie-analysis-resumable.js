...
async function processMovieAnalysisContent(content, currentMovieTitle = '', tmdbCache) {
  if (!content || typeof content !== 'string') return { content, stats: { total: 0, linked: 0, stripped: 0 } };
...
async function processBatch(files, batchStart, batchEnd, progress) {
  ...
  for (let i = 0; i < batchFiles.length; i++) {
    ...
    if (result.stats.total > 0) {
      console.log(`  ✅ ${result.stats.linked} linked, ${result.stats.stripped} stripped (${result.stats.total} total)`);
    }
  }
...
async function processAllNuclearStaticFiles() {
  ...
  if (progress.processedFiles) {
    ...
    console.log(`📈 Resuming from previous session: ${progress.processedFiles.size} files already processed`);
  ...
  const successRate = progress.totalStats.totalPatterns > 0 
