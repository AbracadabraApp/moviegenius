...
async function processMoviePage(page, tmdbId, index) {
  ...
  try {
    ...
    if (CONFIG.WAIT_FOR_ANALYSIS) {
      ...
      try {
        // Wait for content generation indicators
        await Promise.race([
        ...
        if (pageContent.hasExploreFurther) {
          log(`   ✅ ${tmdbId}: "${movieTitle}" - Fresh content with ${pageContent.exploreFurtherCount} explore topics (${responseTime}ms)`);
          stats.fresh++;
        ...
        const responseTime = Date.now() - startTime;
        log(`   ⚠️  ${tmdbId}: "${movieTitle}" - Analysis timeout but page loaded (${responseTime}ms)`);
        stats.cached++;
      }
    }
    ...
    if (CONFIG.SAVE_SCREENSHOTS) {
      try {
        ...
        log(`   ⚠️  Screenshot failed for ${tmdbId}`);
      }
    }
...
async function main() {
  ...
  try {
    ...
    // Create directories
    if (CONFIG.SAVE_SCREENSHOTS && !fs.existsSync('./screenshots')) {
      fs.mkdirSync('./screenshots');
    }
    ...
    log(`✅ Successfully processed: ${stats.successful} (${((stats.successful / stats.total) * 100).toFixed(1)}%)`);
