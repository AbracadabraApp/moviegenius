...
async function testDiscoveryFlow() {
  ...
  for (const query of testQueries) {
    ...
    try {
      ...
      if (dbResult) {
        console.log(`✅ Found in database: "${dbResult.title}" (${dbResult.year}) -> /movie/${dbResult.tmdb_id}`);
      ...
      if (tmdbResult.type === 'single_match') {
        console.log(`✅ Single TMDB match: "${tmdbResult.movie.title}" -> /movie/${tmdbResult.movie.id}`);
        ...
        tmdbResult.results.slice(0, 3).forEach((movie, i) => {
          ...
          console.log(`  ${i + 1}. "${movie.title}" (${year}) - Score: ${movie.relevanceScore?.toFixed(1) || 'N/A'}`);
        });
