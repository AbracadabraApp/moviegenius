...
async function extractTrailerData() {
  ...
  try {
    ...
    for (const line of lines) {
      ...
      const noTrailerMatch = line.match(/📝\s*\[(\d+)\]\s*(\d+)\s*-\s*Description movie but no trailer!/);
      if (noTrailerMatch) {
    ...
    // Analyze ID ranges
    const allIds = [...moviesWithTrailers.map(m => m.movieId), ...moviesWithoutTrailers.map(m => m.movieId)];
