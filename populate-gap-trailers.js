...
async function getMoviesNeedingTrailers() {
  ...
  const needTrailers = moviesWithAnalysis.filter(movie => 
    !movie.trailer_url || movie.trailer_url.trim() === ''
  );

  console.log(`📊 Analysis Results:`);
  console.log(`Total movies with analysis (TMDB ID ≤ ${MAX_TMDB_ID}): ${moviesWithAnalysis.length}`);
      ...
      for (const movie of batch) {
        ...
        try {
          ...
          if (trailerId) {
            ...
            foundCount++;
            console.log(`  ✅ [${processedCount}] ${movie.title} (${movie.year}) - NEW: ${trailerId}`);
          } else {
            console.log(`  📝 [${processedCount}] ${movie.title} (${movie.year}) - No trailer found`);
          }
