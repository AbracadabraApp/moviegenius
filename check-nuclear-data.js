...
async function checkDataCompleteness() {
  ...
  moviesWithAnalyses.forEach((movie, index) => {
    ...
    // Check poster
    if (movie.poster_url && movie.poster_url !== '/images/placeholder-poster.jpg' && movie.poster_url.includes('image.tmdb.org')) {
      goodPosters++;
    } else {
      issues.push('poster');
    }
  ...
  console.log(`   📸 Good Posters: ${goodPosters}/${total} (${(goodPosters/total*100).toFixed(1)}%)`);
  console.log(`   📝 Good Slugs: ${goodSlugs}/${total} (${(goodSlugs/total*100).toFixed(1)}%)`);
  console.log(`   📺 Streaming Data: ${hasStreaming}/${total} (${(hasStreaming/total*100).toFixed(1)}%)`);
  console.log(`   🔍 Explore Further: ${hasExploreFurther}/${total} (${(hasExploreFurther/total*100).toFixed(1)}%)`);
  console.log(`   🎬 More Ideas: ${hasMoreIdeas}/${total} (${(hasMoreIdeas/total*100).toFixed(1)}%)`);
  ...
  if (readyForNuclear >= 5000) {
    ...
    console.log(`   ⚠️  PARTIAL: ${readyForNuclear} movies ready, ${5000 - readyForNuclear} need enhancement`);
  }
}
