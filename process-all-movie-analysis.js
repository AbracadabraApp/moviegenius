...
async function processAllNuclearStaticFiles() {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if (result.stats.total > 0) {
      console.log(`  ✅ ${result.stats.linked} linked, ${result.stats.stripped} stripped (${result.stats.total} total patterns)`);
    }
  ...
  console.log(`   • Success rate: ${totalStats.totalPatterns > 0 ? Math.round((totalStats.totalLinked / totalStats.totalPatterns) * 100) : 0}%`);
