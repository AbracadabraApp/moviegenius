  ...
  try {
    ...
    if (missingNuclear.length > 0) {
      ...
      missingNuclear.slice(0, 10).forEach((movie, index) => {
        console.log(
          `     ${index + 1}. ${movie.movies.title} (${movie.movies.year}) - TMDB: ${movie.movies.tmdb_id}`
        );
      });
    }
    ...
    const overallConversionRate =
      (completeNuclear.length / Math.max(allAnalyzedMovies.length, nuclearCandidates.length)) * 100;
    ...
    if (missingNuclear.length > 0) {
      ...
      console.log(
        `   Command: node scripts/nuclear-static-generator.js --tmdb-ids=${missingNuclear
          .slice(0, 20)
          .map(m => m.movies.tmdb_id)
          .join(',')}`
      );
    }
    ...
    if (candidatesNeedingAnalysis.length > 0) {
      ...
      console.log(
        `   Command: node scripts/batch-analysis-generator.js --tmdb-ids=${candidatesNeedingAnalysis
          .slice(0, 20)
          .map(m => m.tmdb_id)
          .join(',')}`
      );
