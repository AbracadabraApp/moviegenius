...
async function processMovieAnalysisContent(content, filename = '') {
  if (!content || typeof content !== 'string') return { content, stats: { total: 0, found: 0, notFound: 0 } };
  ...
  for (const mention of mentions.reverse()) {
    ...
    if (movieData) {
      ...
      console.log(`✅ Found "${mention.title}" → TMDB ID ${movieData.tmdb_id} (${movieData.title})`);
      stats.found++;
    } else {
      console.log(`❌ No TMDB match for "${mention.title}" ${mention.year ? `(${mention.year})` : ''}`);
      stats.notFound++;
    }
...
async function testMovieAnalysisSystem(maxFiles = 20) {
      ...
      if (movieData.props?.sections) {
        ...
        if (combinedContent.trim()) {
          ...
          if (result.stats.total > 0) {
            console.log(`📊 ${filename}: ${result.stats.found}/${result.stats.total} matches found`);
          }
        } else {
  ...
  console.log(`  • Success rate: ${totalStats.total > 0 ? Math.round((totalStats.found / totalStats.total) * 100) : 0}%`);
...
export { extractMovieMentions, searchMovieInTMDB, processMovieAnalysisContent, testMovieAnalysisSystem };