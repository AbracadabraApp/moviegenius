...
class AnalysisSlugGenerator {
  ...
  async getMoviesNeedingSlugs() {
    ...
    try {
      ...
      for (let i = 0; i < uniqueMovieIds.length; i += batchSize) {
        ...
        console.log(
          `📋 Batch ${Math.floor(i / batchSize) + 1}: Found ${batchData.length} movies without slugs`
        );
  ...
  async generateSlugForMovie(movie, retryCount = 0) {
    try {
      ...
      console.log(
        `🎬 [${this.stats.processed + 1}] Generating slug for "${movie.title}" (${movie.year})`
      );
      ...
      if (result.slug && !result.error) {
        ...
        console.log(
          `✅ [${this.stats.processed}] "${movie.title}" (${movie.year}) → "${result.slug}"`
        );
        return 'generated';
      } else {
        console.log(
          `⚠️ [${this.stats.processed}] No slug generated for "${movie.title}" (${movie.year}): ${result.error || 'Unknown error'}`
        );
        this.stats.errors++;
        return 'failed';
      }
      ...
      if (retryCount < MAX_RETRIES) {
        console.log(
          `🔄 [${this.stats.processed}] "${movie.title}" - Retry ${retryCount + 1}/${MAX_RETRIES}`
        );
        ...
        this.stats.errors++;
        console.error(
          `💥 [${this.stats.processed}] "${movie.title}" - Failed after ${MAX_RETRIES} retries: ${error.message}`
        );
        return 'error';
      }
    }
  ...
  async run() {
    ...
    const remainingMovies = this.moviesNeedingSlugs.filter(
      movie => !this.progress.processedMovies?.includes(movie.id)
    );
  ...
  printFinalSummary() {
    ...
    console.log(`  • Errors: ${this.stats.errors}`);
    console.log(
      `  • Success rate: ${((this.stats.generated / this.stats.processed) * 100).toFixed(1)}%`
    );
