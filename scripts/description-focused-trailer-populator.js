...
class DescriptionFocusedPopulator {
  ...
  async getMoviesWithDescriptions() {
    ...
    try {
      ...
      console.log(
        `📋 Targeting ${knownQualityMovies.length} movies from TMDB ID ${startId} to ${startId + batchSize - 1}`
      );
    ...
    try {
      ...
      if (result.videoId) {
        if (result.source === 'cache') {
          ...
          this.stats.found++;
          console.log(
            `✅ [${this.stats.processed}] ${tmdbId} - NEW: ${result.videoId.substring(0, 8)}... (${phase})`
          );
        }
        return 'found';
      } else {
      ...
      if (retryCount < MAX_RETRIES) {
        console.log(
          `🔄 [${this.stats.processed}] ${tmdbId} - Retry ${retryCount + 1}/${MAX_RETRIES}`
        );
        ...
        this.stats.errors++;
        console.error(
          `💥 [${this.stats.processed}] ${tmdbId} - Failed after ${MAX_RETRIES} retries`
        );
        return 'error';
      }
    }
  ...
  printProgress() {
    ...
    const successRate = (
      ((this.stats.found + this.stats.cached) / this.stats.processed) *
      100
    ).toFixed(1);
  ...
  async runDescriptionPhase() {
    ...
    const remainingDescriptionMovies = this.moviesWithDescriptions.filter(
      id => !processedArray.includes(id)
    );

    console.log(
      `📋 ${remainingDescriptionMovies.length} description movies remaining to process\n`
    );
  ...
  async runRandomPhase() {
    ...
    const remainingRandomMovies = randomIds.filter(
      id => !(this.progress.processedIds || []).includes(id)
    );
  ...
  printFinalSummary() {
    ...
    console.log(`  • Errors: ${this.stats.errors}`);
    console.log(
      `  • Success rate: ${((totalWithTrailers / this.stats.processed) * 100).toFixed(1)}%`
    );
    ...
    console.log(
      `🔄 Can resume anytime with: node scripts/description-focused-trailer-populator.js`
    );
  }
}
