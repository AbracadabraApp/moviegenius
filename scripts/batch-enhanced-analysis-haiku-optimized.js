...
const MODEL_CONFIG = {
  ...
  concurrency: Number(process.env.CONCURRENCY || 20), // 20x concurrency for maximum speed
  speed: 'fastest',
  quality: 'good'
};
...
const BATCH_PHASES = [
  ...
  { name: 'Large Batch', size: 5000, description: 'Performance scaling test' },
...
// Progress tracking
const PROGRESS_FILE = 'enhanced-analysis-progress-optimized.json';
const RESULTS_FILE = 'enhanced-analysis-results.json';

console.log(`🎬 Enhanced Analysis Batch Generation - MAXIMUM SPEED`);
console.log(`🤖 Model: ${MODEL_CONFIG.displayName}`);
console.log(`⚡ Concurrency: ${MODEL_CONFIG.concurrency}x (NO DELAYS)`);
...
/**
 * Generate enhanced analysis for a movie with retry logic
 */
async function generateEnhancedAnalysis(movie, retryCount = 0) {
  ...
  try {
    ...
    if (retryCount < MAX_RETRIES && (
      ...
      console.log(`   ⚠️  Retry ${retryCount + 1}/${MAX_RETRIES} after ${delay}ms: ${error.message}`);
      await new Promise(resolve => setTimeout(resolve, delay));
      return generateEnhancedAnalysis(movie, retryCount + 1);
    }
...
/**
 * Process a batch of movies with maximum concurrency
 */
async function processBatch(movies, batchIndex, progress) {
  console.log(`\n🎬 Processing batch ${batchIndex}: ${movies.length} movies with ${MODEL_CONFIG.concurrency}x concurrency`);

  const concurrency = MODEL_CONFIG.concurrency;
  const batchResults = [];

  // Process in chunks with maximum concurrency (no delays)
  for (let i = 0; i < movies.length; i += concurrency) {
    const chunk = movies.slice(i, i + concurrency);
    const chunkNo = Math.floor(i / concurrency) + 1;
    const chunkStartTime = Date.now();

    console.log(`  🚀 Chunk ${chunkNo}: Processing ${chunk.length} movies concurrently...`);

    // Process chunk with maximum concurrency
    const chunkPromises = chunk.map(async (movie, chunkIndex) => {
      const globalIndex = progress.processed + i + chunkIndex + 1;
      ...
      try {
        ...
        if (result.success) {
          ...
          console.log(`   ✅ [${globalIndex}] ${movie.title} (${movie.year}) - ${result.analysis.content.length} sections, ${result.metadata.wordCount} words`);
          ...
          console.log(`   ❌ [${globalIndex}] ${movie.title} (${movie.year}) - ${result.error}`);
          progress.failed++;
        ...
        console.log(`   💥 [${globalIndex}] ${movie.title} (${movie.year}) - ${error.message}`);
        progress.failed++;
        progress.processed++;
        progress.processedIds.add(movie.id);
        progress.errors.push({
        ...
        return { success: false, error: error.message, metadata: { cost: 0 } };
      }
    });
    ...
    const avgTime = chunkTime / chunk.length;
    console.log(`  ✅ Chunk ${chunkNo} completed in ${(chunkTime / 1000).toFixed(1)}s (avg: ${avgTime.toFixed(0)}ms/movie)`);
    ...
    const avgTimePerChunk = chunkTime;
    const eta = Math.round((remaining / concurrency) * avgTimePerChunk / 1000);

    console.log(`  📊 Batch ${batchIndex} Progress: ${progress.processed - (progress.processed - movies.length)} processed | ETA: ${eta}s\n`);
  }
