#!/usr/bin/env node
/**
 * Backfill Missing Posters
 * Only fetches posters for movies that don't have them
 * Run: node --env-file=.env.local scripts/backfill-missing-posters.cjs
 */

const { Pool } = require('pg');

async function backfillMissingPosters() {
  console.log('🎬 Starting poster backfill for movies with missing posters...\n');
    ...
    for (const movie of movies) {
      try {
        ...
        if (tmdbData.poster_path) {
          ...
          // Update database
          await client.query(
            'UPDATE movies SET poster_url = $1 WHERE tmdb_id = $2',
            [posterUrl, movie.tmdb_id]
          );
          console.log(`   ✅ Found poster: ${tmdbData.poster_path}`);
          found++;
        } else {
          console.log(`   ⚠️  No poster found on TMDB`);
        }
        ...
        if (processed % 50 === 0) {
          ...
          console.log(`🎯 Posters found: ${found} (${(found/processed*100).toFixed(1)}% success rate)`);
...
backfillMissingPosters().catch(error => {
