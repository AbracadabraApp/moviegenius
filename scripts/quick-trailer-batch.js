  ...
  try {
    ...
    if (result.videoId) {
      if (result.source === 'cache') {
        cached++;
        console.log(`🚀 [${processed}/${popularMovieIds.length}] TMDB ${tmdbId} - Cached: ${result.videoId}`);
      } else {
        found++;
        console.log(`✅ [${processed}/${popularMovieIds.length}] TMDB ${tmdbId} - Found: ${result.videoId}`);
      }
    } else {
    ...
    console.error(`💥 [${processed}/${popularMovieIds.length}] TMDB ${tmdbId} - Error:`, error.message);
  }
}
