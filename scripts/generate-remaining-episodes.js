...
function generateContentSections(episode, topic) {
  ...
  const sections = [
    ...
    },
    {
      ...
      prompts: [`How did the techniques discussed in ${episode.title} influence later filmmakers?`],
    },
    {
      type: 'subhead',
...
function generateEpisodes(dryRun = false) {
  ...
  episodesToGenerate.forEach(
    ({ themeId, seriesId, episodeId, theme, series, episode, filePath }) => {
      try {
