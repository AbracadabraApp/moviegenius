...
function processTextForMovieLinks(text) {
  ...
  if (matches.length > 0) {
    ...
    for (const movieMatch of matches.reverse()) {
      ...
      processedText =
        processedText.slice(0, movieMatch.start) + link + processedText.slice(movieMatch.end);
      linksCreated++;
    }
...
function initializeProgress() {
  if (fs.existsSync(PROGRESS_PATH)) {
    try {
      ...
      console.log(
        `📊 Loaded existing progress: ${progressData.completed.length} completed, ${progressData.failed.length} failed`
      );
    } catch (error) {
  ...
  for (const movie of movies) {
    try {
      ...
      if (tmdbData.tmdb_id) {
        ...
        movie.poster_url =
          tmdbData.poster !== '/images/placeholder-poster.jpg' ? tmdbData.poster : null;
...
async function tryLookupMovieEndpoint(movie) {
  try {
    ...
    if (response.ok) {
      ...
      if (lookupData.tmdb_id) {
        ...
        console.log(
          `   ✅ ${movie.title} (${movie.year}) → TMDB ID ${lookupData.tmdb_id} (via lookup-movie)`
        );
        return;
      }
    }
  ...
  movie.tmdb_id = null;
  console.log(
    `   ⚠️  Will rely on MediaCard auto-enhancement for "${movie.title}" (${movie.year})`
  );
}
...
async function withRetry(fn, context, maxRetries = CONFIG.MAX_RETRIES) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.log(
        `   ⚠️  ${context} attempt ${attempt}/${maxRetries} failed, retrying in ${delay}ms...`
      );
...
function validateEpisodeContent(content, episodeNumber) {
  ...
  if (content.moreIdeas.movies.length < 10) {
    issues.push(
      `Only ${content.moreIdeas.movies.length} more_ideas movies (GENIUS requires exactly 10)`
    );
  }

  // Check movie data quality
  const allMovies = [...movieSections.flatMap(s => s.movies), ...content.moreIdeas.movies];
...
async function generateAllEpisodes(options = {}) {
  ...
  if (validateOnly) {
      ...
      if (fs.existsSync(filePath)) {
        try {
          ...
          if (validation.issues.length > 0) {
            ...
            console.log(
              `✅ ${episodeNumber}: ${validation.wordCount} words, ${validation.movieCount} movies`
            );
          }
        } catch (error) {
    ...
    console.log(
      `   Average word count: ${Math.round(validationResults.reduce((acc, r) => acc + r.wordCount, 0) / validationResults.length)}`
    );
    return;
  }
  ...
  console.log(
    `⚠️  This will take approximately ${Math.ceil((allEpisodes.length * (CONFIG.RATE_LIMIT_MS + 3000)) / 60000)} minutes for all ${allEpisodes.length} episodes`
  );
  console.log('');
  ...
  if (startFrom) {
    ...
    console.log(
      `🔄 Starting from episode ${startFrom} (index ${startIndex + 1}/${allEpisodes.length})`
    );
  }
  ...
  for (let i = startIndex; i < allEpisodes.length; i++) {
    ...
    try {
      ...
      console.log(
        `   📝 ${validation.wordCount} words | 🎬 ${validation.movieCount} films | 🔍 ${claudeContent.sections.filter(s => s.type === 'explore_further').length} explore sections`
      );
