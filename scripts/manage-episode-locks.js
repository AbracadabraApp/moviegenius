...
function lockEpisode(episodeId, lockedBy = 'script') {
  ...
  try {
    ...
    if (episodeData.locked) {
      console.log(
        `⚠️  Episode ${episodeId} is already locked (locked at: ${episodeData.lockedAt})`
      );
      return true;
    }
...
function listAllLocked() {
  const files = fs
    ...
    .filter(f => f.startsWith('genius-') && f.endsWith('.json'));
  const locked = [];
...
function lockSeries(seriesPattern) {
  const files = fs
    .readdirSync(EPISODES_DIR)
    .filter(f => f.startsWith(`genius-${seriesPattern}`) && f.endsWith('.json'));
  let success = 0;
  let total = 0;
  ...
  case 'list-locked':
    ...
    if (lockedEpisodes.length === 0) {
      ...
      lockedEpisodes.forEach(ep => {
        console.log(
          `   ${ep.episodeId}: "${ep.title}" (locked by ${ep.lockedBy} at ${ep.lockedAt})`
        );
      });
    }
    break;
