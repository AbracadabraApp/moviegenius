...
async function countAnalysis() {
  ...
  try {
    ...
    console.log(
      `✅ Movies with Analysis: ${moviesWithAnalysis.toLocaleString()} (${analysisPercentage}%)`
    );
    console.log(
      `❌ Movies without Analysis: ${moviesWithoutAnalysis.toLocaleString()} (${noAnalysisPercentage}%)`
    );
    console.log('');
