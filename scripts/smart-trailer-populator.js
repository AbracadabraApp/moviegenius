...
class SmartTrailerPopulator {
    ...
    try {
      ...
      if (result.videoId) {
        if (result.source === 'cache') {
          ...
          this.stats.found++;
          console.log(
            `✅ [${this.stats.processed}] ${tmdbId} - NEW: ${result.videoId.substring(0, 8)}...`
          );
        }
        return 'found';
      } else {
  ...
  async run() {
    ...
    const remainingIds = this.movieIds.filter(
      id => id > this.progress.lastProcessed && !this.progress.processedIds.has(id)
    );
