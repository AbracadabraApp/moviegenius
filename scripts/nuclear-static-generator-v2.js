...
async function main() {
  ...
  for (let i = 0; i < movieIds.length; i++) {
    ...
    if ((i + 1) % 50 === 0) {
      ...
      console.log(
        `⏱️  ${elapsed.toFixed(0)}s elapsed, ${eta.toFixed(0)}s remaining, ${rate.toFixed(1)} movies/sec\n`
      );
    }
  }
  ...
  console.log(
    `⏱️  Total time: ${totalTime.toFixed(0)}s (${(successCount / totalTime).toFixed(1)} movies/sec)`
  );
}
