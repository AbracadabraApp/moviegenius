...
async function analyzeSlugDistribution() {
  ...
  Object.entries(buckets).forEach(([bucket, count]) => {
    ...
    console.log(
      `   ${bucket.padEnd(18)}: ${count.toString().padStart(4)} (${percentage.padStart(5)}%) ${bar}`
    );
  });
  ...
  console.log('\n🎯 QUALITY ASSESSMENT:');
  console.log(
    `   ✅ Good slugs (≤50 chars): ${goodSlugs}/${claudeSlugs.length} (${((goodSlugs / claudeSlugs.length) * 100).toFixed(1)}%)`
  );
  console.log(
    `   🔴 Problem slugs (>50 chars): ${problemSlugs}/${claudeSlugs.length} (${((problemSlugs / claudeSlugs.length) * 100).toFixed(1)}%)`
  );
}
