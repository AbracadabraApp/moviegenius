...
`;
  ...
  async processMovie(page, movie, index) {
    ...
    try {
      ...
      this.log(
        `✅ SUCCESS: [${movieNumber}] ${movie.title} (${(duration / 1000).toFixed(1)}s, ${method}) ${analysisStatus} ${analysisCheck.contentLength} chars`
      );
      this.processedCount++;
      ...
      const duration = Date.now() - startTime;
      this.log(
        `❌ ERROR: [${movieNumber}] ${movie.title} - ${error.message} (${(duration / 1000).toFixed(1)}s)`
      );
      this.errorCount++;
  ...
  adjustConcurrency() {
    const recentErrorRate =
      ...
      Math.min(CONFIG.errorWindow, this.processedCount + this.errorCount);
    ...
    if (recentErrorRate <= 0.1 && this.currentTabs < CONFIG.maxTabs) {
      ...
      this.currentTabs++;
      this.log(
        `📈 SCALING UP: Increased to ${this.currentTabs} tabs (${(recentErrorRate * 100).toFixed(1)}% error rate)`
      );
    } else if (recentErrorRate > 0.2 && this.currentTabs > 1) {
      // High error rate, scale down
      this.currentTabs = Math.max(1, this.currentTabs - 1);
      this.log(
        `📉 SCALING DOWN: Reduced to ${this.currentTabs} tab(s) (${(recentErrorRate * 100).toFixed(1)}% error rate)`
      );
  ...
  printProgress() {
    ...
    this.log(`\n📊 PROGRESS REPORT:`);
    this.log(
      `   ✅ Completed: ${completed}/${this.totalMovies} (${((completed / this.totalMovies) * 100).toFixed(1)}%)`
    );
    this.log(
      `   🎯 Success Rate: ${successRate.toFixed(1)}% (${this.processedCount} success, ${this.errorCount} errors)`
    );
    this.log(
      `   🧠 Analysis Generated: ${this.analysisGeneratedCount}/${this.processedCount} (${analysisRate.toFixed(1)}%)`
    );
    this.log(
      `   ⚡ Smart Detection: ${this.smartContentCount} | ⏰ Timeouts: ${this.timeoutFallbackCount}`
    );
    this.log(`   🚀 Current Tabs: ${this.currentTabs}`);
    this.log(
      `   📈 Rate: ${rate.toFixed(2)} movies/sec | 🕐 ETA: ${(eta / 60).toFixed(1)} minutes\n`
    );
  }
  ...
  async run() {
    try {
      ...
      this.log(
        `🎯 Processing ${movies.length - this.resumeIndex} movies (starting from #${this.resumeIndex + 1})`
      );
      this.log(
        `⚙️  Config: ${this.currentTabs} initial tabs, ${CONFIG.maxTimeout / 1000}s timeout, ${CONFIG.interval / 1000}s intervals\n`
      );
  ...
  printFinalReport() {
    ...
    this.log(
      `   🧠 Analysis Generated: ${this.analysisGeneratedCount} (${analysisRate.toFixed(1)}%)`
    );
    this.log(
      `   ⚡ Smart Detection: ${this.smartContentCount} | ⏰ Timeouts: ${this.timeoutFallbackCount}`
    );
