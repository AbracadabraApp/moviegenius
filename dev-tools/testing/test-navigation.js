...
function testHomepageThemeNavigation() {
  ...
  expectedThemes.forEach(theme => {
    if (!themeRoutes[theme]) {
      addError(
        `Homepage theme "${theme}" missing route mapping`,
        'route exists',
        'route missing',
        'pages/index.js'
      );
...
function testThemePageEpisodeNavigation() {
  ...
  const essentialMoviesContent = readComponentFile(
  ...
  if (!episodeLinkPattern.test(essentialMoviesContent)) {
    addError(
      'EssentialMovies episode link pattern',
...
function testEpisodePageThemeNavigation() {
  ...
  const episodeTemplateContent = readComponentFile(
  ...
  expectedThemes.forEach(theme => {
    if (!themeRoutes[theme]) {
      addError(
        `Episode page theme "${theme}" missing route mapping`,
        'route exists',
        'route missing',
    ...
    if (!route.startsWith('/themes/')) {
      addError(
        `Episode page theme "${theme}" route`,
        'starts with /themes/',
        route,
        'components/GeniusEpisodeTemplate.js'
      );
...
function testThemeFooterNavigation() {
  ...
  const themeFooterContent = readComponentFile(
...
function testThemePageExistence() {
  ...
  Object.keys(themeMapping.themes).forEach(themeId => {
    ...
    if (!fs.existsSync(themePagePath)) {
      addError(
        `Theme page file "${themeId}"`,
        'file exists',
        'file missing',
        `pages/themes/${themeId}.js`
      );
      return;
    }
