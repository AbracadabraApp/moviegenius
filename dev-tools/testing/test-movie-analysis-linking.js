...
async function processAnalysisContent(content, currentMovieTitle = '', context = '') {
  ...
  console.log(
    `✅ Created ${linksCreated} links, stripped ${strippedCount} marks, made ${tmdbLookups} TMDB lookups in ${context}`
  );
...
async function testSampleContent() {
  ...
  mentions.forEach((mention, i) => {
    console.log(
      `  ${i + 1}. ${mention.type}: "${mention.title}"${mention.year ? ` (${mention.year})` : ''}`
    );
  });
