...
export default function MovieDetailPage({ movieId, movieData }) {
  ...
  const streaming = movieData?.movieHeader?.streaming || null;
  const analysis = movieData?.analysis || null;
  const keyElements = movieData?.keyElements || null;
  const error = null;

  // No client-side data fetching - all data comes from static generation

  // All movie data is pre-loaded from static generation
