...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (existingAnalysis && !analysisError) {
      ...
      console.log(`Using cached analysis for ${title} (${year})`);
      // Cache for 1 week since content is stable
    ...
    console.log(`Generating new analysis for ${title} (${year})`);
    ...
    try {
      ...
      console.warn('Entity detection stack:', entityError.stack);
    ...
    // Cache newly generated content for 1 week
    res.setHeader('Cache-Control', 'public, s-maxage=604800, stale-while-revalidate=1209600');
    console.timeEnd(`🎬 API analysis for ${title} (${year})`);
    res.status(200).json({ 
      analysis: analysis,
