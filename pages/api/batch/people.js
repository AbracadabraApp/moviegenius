...
class RailwayPeopleBatchProcessor {
  ...
  isProbablyPersonName(name) {
    const excludeWords = [
      'New York', 'Los Angeles', 'United States', 'World War',
      'Golden Age', 'Great Depression', 'Cold War', 'Film Noir',
      'French New', 'Italian Neo', 'German Expressionism'
    ];
    
    return !excludeWords.some(excluded => name.includes(excluded)) && 
           name.length > 5 && 
           name.length < 30;
  }
  ...
  calculateNameConfidence(name, content) {
    ...
    if (content.includes(`${name} directed`) || 
      ...
      const { data: newPerson, error } = await supabase
        ...
        .insert({
          ...
          profile_url: personData.profile_path ? 
            `https://image.tmdb.org/t/p/w500${personData.profile_path}` : null,
