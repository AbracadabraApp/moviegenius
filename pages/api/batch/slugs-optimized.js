...
Return ONLY the tagline, nothing else.`;
  ...
  async processSlugsInParallel(movies) {
      ...
      },
      {
        ...
        onProgress: progress => {
          if (progress.completed % 10 === 0) {
            console.log(
              `📊 Slug generation progress: ${progress.percentage}% (${progress.completed}/${progress.total})`
            );
          }
        },
      }
    ...
    console.log(
      `✅ Slug generation completed: ${result.results.length} successful, ${result.errors.length} failed`
    );
  ...
  async processBatch() {
    ...
    try {
      ...
      console.log(
        `📊 ${totalSucceeded}/${totalProcessed} successful (${successRate}%) in ${totalDuration}ms`
      );
