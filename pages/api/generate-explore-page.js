...
NEXT_STEPS: specific suggestion 1|specific suggestion 2|specific suggestion 3`;
      ...
      try {
        ...
        console.log(`💾 Generated static page for: "${topic}" (${parsedPage.sections.length} sections)`);
        return parsedPage;
...
function parseExplorePage(responseText, topic = 'Film Topic') {
  ...
  for (const line of lines) {
    ...
    if (trimmedLine.startsWith('TITLE:')) {
      ...
      if (stepsLine) {
        const steps = stepsLine.split('|').map(s => s.trim()).filter(s => s);
...
function generateFallbackExplorePage(topic, context) {
  return {
    ...
    movies: [
      { title: 'Citizen Kane', year: 1941, slug: 'Foundational cinema techniques', streaming: 'Free on Archive.org' },
      { title: 'Vertigo', year: 1958, slug: 'Masterclass in visual storytelling', streaming: 'Free on Kanopy' },
      { title: 'The Godfather', year: 1972, slug: 'Influential narrative structure', streaming: 'Rent on Prime Video' }
    ],
    nextSteps: [
...
async function generateExplorePageHandler(req, res) {
  ...
  try {
    ...
    const response = successResponse(
      pageContent,
      'Explore page generated successfully'
    );
