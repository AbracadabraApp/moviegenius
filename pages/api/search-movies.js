...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log(`✅ Enhanced search success: "${query.trim()}" -> ${movies.length} results (${movies.filter(r => r.type === 'movie').length} movies, ${movies.filter(r => r.type === 'person').length} people)`);
