  ...
  try {
    ...
    try {
      ...
      for (const movieMention of movieMentions) {
        try {
          const createResponse = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/api/create-media-card`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ 
          ...
          console.warn(`⚠️ Failed to create MediaCard for ${movieMention.title}:`, movieError.message);
        }
      }
