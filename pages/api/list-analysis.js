...
export default async function handler(req, res) {
  ...
  try {
    ...
    const promptConfig = buildPrompt(
      'LIST',
      'Generate concise collection description and curated movie list (15-25 films). Keep descriptions brief and focused on curation context.'
    );
        ...
        if (parts.length >= 2) {
          ...
          try {
            ...
            if (mediaCardResult.success) {
              console.log(
                `✅ Created MediaCard for ${movieData.title} (${movieData.year}) from list analysis`
              );
              ...
              console.log(
                `⚠️ Failed to create MediaCard for ${movieData.title} (${movieData.year}):`,
                mediaCardResult.error
              );
    ...
    if (saveError) {
      ...
      console.log(
        `Saved combined analysis for list ${listName} - ${movies.length} movies - Cost: $${costEstimate.toFixed(4)}`
      );
    }
