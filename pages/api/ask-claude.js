  ...
  try {
    ...
    if (existingMovie) {
      ...
      if (!existingMovie.slug && movieData.slug) {
        ...
        // Enhanced existing movie with slug
        return data;
      } else {
        // Movie already has slug, skipping
        return existingMovie;
      }
    } else {
      // Step 3: Movie doesn't exist - do full TMDB lookup and create complete record
      // New movie discovered
      ...
      if (tmdbData?.tmdb_id) {
        ...
        if (existingTmdbMovie) {
          // Movie already exists with TMDB ID
...
async function fetchFullTMDBData(title, year) {
  ...
  return await cache.cacheTMDBResponse('search_movie', { title, year }, async () => {
    // Cache miss - fetching TMDB data
...
async function checkRequestDeduplication(question) {
  ...
  if (pendingRequests.has(requestKey)) {
    ...
    // Request deduplication: Waiting for existing request

    try {
      // Wait for existing request to complete
      const result = await existingRequest.promise;
      // Request deduplication: Got cached result
      return result;
    } catch (error) {
      // If existing request failed, allow this one to proceed
      // Request deduplication: Existing request failed, proceeding
 ...
 */
async function generateClaudeResponse(question, conversationContext = null) {
  const monitor = getPerformanceMonitor();
  const cache = getCache();

  // Check for 'yes' responses and short affirmations
  const isYesResponse =
    /^(yes|yeah|yep|sure|definitely|absolutely|exactly|right|correct|true|indeed|please)\.?$/i.test(
      question.trim()
    );
  const isShortAffirmation =
    question.length <= 10 &&
    /^(yes|y|yeah|yep|sure|ok|okay|right|true|exactly|definitely|absolutely|indeed|please|more|continue)\.?$/i.test(
      question.trim()
    );

  // If it's a yes response without context, provide guidance
  if ((isYesResponse || isShortAffirmation) && !conversationContext) {
    return {
      sections: [
        {
          type: 'text',
          content:
            "I'd love to help! Could you tell me what kind of movies you're interested in? For example, are you looking for action films, comedies, dramas, or something else?",
        },
      ],
      followUpQuestion: 'What genre or type of movie are you in the mood for?',
      followUpQuestions: ['What genre or type of movie are you in the mood for?'],
      moreIdeas: null,
    };
  }
  ...
  if (predictiveResult) {
    // INSTANT response from predictive cache
  ...
  // Create normalized key for pending request tracking (include context for uniqueness)
  const contextKey = conversationContext
    ? `_ctx_${conversationContext.lastQuestion}_${conversationContext.lastResponse?.substring(0, 50) || ''}`
    : '';
  const requestKey = (question.toLowerCase().trim().replace(/\s+/g, '_') + contextKey).substring(
    0,
    150
  );
  ...
  const cachePromise = cache.cacheClaudeResponse(
    ...
    async () => {
      ...
      // Detect follow-up questions and conversation context
      const isFollowUp =
        question.length < 50 ||
      ...
      // Build conversation-aware prompt guidance
      let customGuidance = '';
      if (conversationContext && (isYesResponse || isShortAffirmation)) {
        customGuidance = `The user just responded "${question}" to the previous follow-up question: "${conversationContext.lastFollowUpQuestion}". This means they want to explore that direction. Provide specific film recommendations that expand on that interest, maintaining the conversational tone.`;
      } else if (isFollowUp && conversationContext) {
        customGuidance = `This is a follow-up question in an ongoing conversation. Previous context: "${conversationContext.lastResponse?.substring(0, 100) || ''}". Build naturally on this conversation.`;
      } else {
        customGuidance =
          'Be direct and conversational. Start with acknowledgment then jump into specific film recommendations.';
      }

      // CONVERSATIONAL STRATEGY: Context-aware responses with professor persona
      const promptConfig = buildPrompt('ASK', customGuidance, true);

      // Conversational strategy determined
      ...
      if (conversationContext) {
        messages.push({
          role: 'user',
          content: conversationContext.lastQuestion,
        });
        messages.push({
          role: 'assistant',
          content: conversationContext.lastResponse || 'I provided some movie recommendations.',
        });
        if (conversationContext.lastFollowUpQuestion) {
          messages.push({
            role: 'assistant',
            content: `Follow-up: ${conversationContext.lastFollowUpQuestion}`,
          });
        }
      }
...
function parseClaudeResponse(responseText) {
  // Conversational ASK response debug

  const lines = responseText.split('\n');
  let followUpQuestion = null; // Single question instead of array
  let mainContent = '';

  // Separate main content from follow-up question
  for (const line of lines) {
    const trimmedLine = line.trim();

    if (trimmedLine.startsWith('FOLLOW_UP_QUESTION:')) {
      // Parse single follow-up question (no pipe separation)
      const questionLine = trimmedLine.replace('FOLLOW_UP_QUESTION:', '').trim();
      if (questionLine) {
        followUpQuestion = questionLine;
      }
    } else if (trimmedLine.startsWith('FOLLOW_UP_QUESTIONS:')) {
      // Legacy format support - take first question only
      const questionsLine = trimmedLine.replace('FOLLOW_UP_QUESTIONS:', '').trim();
      if (questionsLine) {
        const firstQuestion = questionsLine.split('|')[0]?.trim();
        if (firstQuestion) {
  ...
  // For conversational Ask format, we return a single text section
  ...
  // Parsed conversational response

  return {
    sections,
    followUpQuestion, // Single question
    followUpQuestions: followUpQuestion ? [followUpQuestion] : [], // Array format for UI compatibility
          ...
          movies: [
            {
              ...
              year: 1941,
              slug: "Bogart's detective navigates web of greed and betrayal",
              streaming: 'Free on Tubi',
            },
            {
              title: 'Double Indemnity',
              year: 1944,
              slug: 'Insurance investigator uncovers seductive murder plot',
              streaming: 'Free on Internet Archive',
            },
            {
              title: 'The Big Sleep',
              year: 1946,
              slug: 'Philip Marlowe untangles complex Los Angeles blackmail',
        ...
        movies: [
          {
            title: 'Laura',
            year: 1944,
            slug: "Detective falls for murdered woman's portrait",
            streaming: 'Free on YouTube',
          },
          {
            title: 'Out of the Past',
            year: 1947,
            slug: "Private eye's past catches up with him",
            streaming: 'Free on Internet Archive',
          },
          {
            title: 'Touch of Evil',
            year: 1958,
            slug: 'Border town corruption investigation',
            streaming: 'Free on Kanopy',
          },
          {
            title: 'The Third Man',
            year: 1949,
            slug: 'Post-WWII Vienna black market conspiracy',
            streaming: 'Free on Tubi',
          },
          {
            title: 'Kiss Me Deadly',
            year: 1955,
            slug: 'Private detective seeks mysterious briefcase',
            streaming: 'Free on Archive.org',
          },
          {
            title: 'Blade Runner',
            year: 1982,
            slug: 'Replicant hunter questions identity in dystopia',
            streaming: 'Rent on Prime Video',
          },
          {
            title: 'Blood Simple',
            year: 1984,
            slug: 'Adultery leads to murder in Texas',
            streaming: 'Free on Crackle',
          },
          {
            title: 'Blue Velvet',
            year: 1986,
            slug: 'College student discovers suburban darkness',
            streaming: 'Free on Kanopy',
          },
          {
            title: 'The Grifters',
            year: 1990,
            slug: 'Con artists family loyalty and betrayal',
            streaming: 'Free on Tubi',
          },
          {
            title: 'Basic Instinct',
            year: 1992,
            slug: 'Detective entangled with murder suspect',
            streaming: 'Rent on Apple TV',
          },
          {
            title: 'The Last Seduction',
            year: 1994,
            slug: 'Femme fatale manipulates small-town insurance man',
            streaming: 'Free on Pluto TV',
          },
          {
            title: 'Mulholland Drive',
            year: 2001,
            slug: 'Hollywood dreams turn nightmarish mystery',
            streaming: 'Free on Kanopy',
          },
          {
            title: 'No Country for Old Men',
            year: 2007,
            slug: 'Drug deal gone wrong pursued by killer',
            streaming: 'Rent on Prime Video',
          },
          {
            title: 'Drive',
            year: 2011,
            slug: 'Stunt driver gets involved with heist',
            streaming: 'Free on Crackle',
          },
          {
            title: 'Nightcrawler',
            year: 2014,
            slug: 'Freelance crime journalist crosses ethical lines',
            streaming: 'Free on Tubi',
          },
          {
            title: 'Gone Girl',
            year: 2014,
            slug: 'Marriage unravels during missing person case',
            streaming: 'Rent on Prime Video',
          },
          {
            title: 'Hell or High Water',
            year: 2016,
            slug: 'Brothers rob banks to save family ranch',
            streaming: 'Free on Crackle',
          },
          {
            title: 'Blade Runner 2049',
            year: 2017,
            slug: 'New blade runner uncovers buried secret',
            streaming: 'Rent on Apple TV',
          },
          {
            title: 'Knives Out',
            year: 2019,
            slug: 'Detective investigates wealthy family murder',
        ...
        movies: [
          {
            title: 'Citizen Kane',
            year: 1941,
            slug: "Media mogul's life story told in flashbacks",
            streaming: 'Free on Internet Archive',
          },
          {
            title: 'Casablanca',
            year: 1942,
            slug: 'Nightclub owner helps resistance in WWII Morocco',
            streaming: 'Free on Tubi',
          },
          {
            title: 'The Godfather',
            year: 1972,
            slug: 'Mafia family patriarch transfers power to son',
            streaming: 'Rent on Prime Video',
          },
        ],
      },
        ...
        movies: [
          {
            title: 'Vertigo',
            year: 1958,
            slug: 'Detective with acrophobia follows mysterious woman',
            streaming: 'Free on Kanopy',
          },
          {
            title: "Singin' in the Rain",
            year: 1952,
            slug: "Silent film stars navigate Hollywood's sound era",
            streaming: 'Free on YouTube',
          },
          {
            title: 'Some Like It Hot',
            year: 1959,
            slug: 'Musicians disguise as women to escape mob',
            streaming: 'Free on Tubi',
          },
        ],
      },
...
async function askClaudeHandler(req, res) {
  ...
  if (!process.env.TMDB_API_KEY) {
    throw ApiErrors.SERVICE_UNAVAILABLE(
      'TMDB service is not configured. Please add your TMDB API key.'
    );
  }
  ...
  try {
    // Generate Claude response with film expertise and conversation context
    const claudeResponse = await generateClaudeResponse(question, conversationContext);
    ...
    // Final response debug - sections and movies processed
