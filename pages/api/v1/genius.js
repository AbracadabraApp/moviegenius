 ...
 * POST /api/v1/genius
 *
 * Builds personalized feed using More Ideas + browse_list collections
 *
 * Request Body:
 * {
 *   savedIds: number[],  // Array of saved movie tmdb_ids
 *   seenIds?: number[]   // Optional array of already-seen movie IDs
 * }
 *
 * Algorithm:
 ...
 *       seedTitle: string,
 *       seedTmdbId: number,
 *       movies: [{ tmdb_id, title, year, poster_url, slug }]
 *     },
 *     {
 *       type: 'collection',
 *       name: string,
...
export default async function handler(req, res) {
  ...
  const allIds = [...new Set(savedIds)].map(id => parseInt(id)).filter(id => Number.isInteger(id) && id > 0);
  ...
  try {
    ...
    const seedData = []; // { seedTmdbId, ideaTmdbIds }
    ...
    console.error('[v1] Genius feed error:', error);
