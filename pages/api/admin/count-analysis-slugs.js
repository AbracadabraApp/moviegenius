...
export default async function handler(req, res) {
  try {
    ...
    if (analysisError) {
      ...
      return res.status(500).json({ error: 'Failed to fetch analysis data', details: analysisError });
    }
    ...
    const moviesWithSlugs = moviesData.filter(movie => movie.slug !== null && movie.slug !== '').length;
    const moviesWithoutSlugs = moviesData.filter(movie => movie.slug === null || movie.slug === '').length;
    ...
    const estimatedMoviesWithoutSlugs = Math.round((moviesWithoutSlugs / moviesData.length) * estimatedUniqueMovies);
    ...
    const result = {
      ...
      insights: {
        ...
        recommendation: slugPercentage < 50 ? 
