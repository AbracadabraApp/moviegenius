...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (existingMovie) {
      ...
      const existingSlugIsGood = existingMovie.slug && existingMovie.slug.length > 30 && !existingMovie.slug.includes('-');
      const newSlugIsBetter = slug && slug.length > 30 && !slug.includes('-');
      
      // 🚨 STRICT FILTER: Block obvious URL-format slugs 
      const isUrlFormatSlug = slug && slug.includes('-') && slug.length < 40 && 
        (slug.match(/^[a-z0-9-]+-(19|20)\d{2}$/) || 
         slug.toLowerCase() === title.toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/^the-|^a-|^an-/, ''));
      ...
      if (streaming && (!existingMovie.streaming_data || streaming !== existingMovie.streaming_data)) {
