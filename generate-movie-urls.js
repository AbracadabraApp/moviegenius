...
async function generateMovieUrls() {
  ...
  try {
    ...
    while (true) {
      ...
      console.log(`📖 Loaded page ${page + 1}: ${movies.length} movies (total: ${allMovies.length})`);
    ...
    const prioritizedMovies = [
      ...
      ...allMovies.filter(movie => 
        !moviesWithoutAnalysis.some(noAnalysis => noAnalysis.tmdb_id === movie.tmdb_id)
      )
    ];
    
    console.log(`📊 Total movies: ${prioritizedMovies.length} (${moviesWithoutAnalysis.length} without analysis)`);
